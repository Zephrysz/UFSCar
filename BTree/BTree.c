...
void split_noh(ArvB *raiz, ArvB *pai) { // 1 to 2
...
void split_noh_raiz(ArvB *raiz) { // caso especifico de split na raiz
  int m = (ordem - 1) >> 1;
...
int arvB_insere(ArvB *raiz, int valor) {
  ...
  if ((*raiz)->filhos[0] == NULL) { // se o noh eh folha
...
int remover(ArvB *raiz, int valor) {
  ...
  if (back == 1 && (*raiz)->qtd == 0) { // se o tratamento de underflow deixou a
...
int arvB_remove(ArvB *raiz, int valor) {
  if ((*raiz) == NULL || (*raiz)->qtd == 0) { // se der falha na remocao
    return 0;
  }
  ...
  while (i < (*raiz)->qtd && (*raiz)->valores[i] <= valor) {
    if ((*raiz)->valores[i] == valor) {
      if ((*raiz)->filhos[0] == NULL) { // se eh folha
        ...
        (*raiz)->qtd--;
        return 1;
      } else { // se n for folha, "troca" com o antecessor imediato
  ...
  if ((*raiz)->filhos[i]->qtd < ordem >> 1) // underflow
  {
    ...
    if (i > 0 &&
        (*raiz)->filhos[i - 1]->qtd > ordem >> 1) { // redistribuicao esq
      redistribuicao(&(*raiz)->filhos[i], &(*raiz)->filhos[i - 1], raiz);
    } else if (i < (*raiz)->qtd &&
               (*raiz)->filhos[i + 1]->qtd > ordem >> 1) { // redistribuicao dir
    ...
    // realizar a concatenacao
                          ...
                          1) { // concatenacao esquerda
      concatena(&(*raiz)->filhos[i], &(*raiz)->filhos[i - 1], raiz);
    } else { // concatenacao direita
...
void redistribuicao(ArvB *filhoDir, ArvB *filhoEsq, ArvB *pai) {
  ...
  for (int i = 0; i < (*filhoEsq)->qtd;
       i++) { // coloca os elementos do filhoEsq no vetor auxiliar
